...
print(f'\n*************************************\nCvičení 2\n*************************************')
hundreds = [i for i in range(200, 2000, 200)]

ascii = [random.choice(string.ascii_uppercase).upper() for i in (1, 50)]
...
print(f'\n*************************************\nCvičení 3\n*************************************')
persons.append(("David", 18, "muž"))
persons.append(("Adam", 23, "muž"))
persons.append(("Zuzana", 17, "žena"))
persons.append(("Nela", 56, "žena"))
print(persons)

women = list(filter(lambda person: person[2] == "žena", persons)) or [person for person in persons if person[2] == "žena"]
print(women)

[print(f"{woman[0]}\n{'-' * len(woman[0])}") for woman in women]

ipeople = list(filter(lambda person: "i" in person[0].lower(), persons)) or [person for person in persons if "i" in person[0].lower()]
print(ipeople)

ipeople.sort(key=lambda x: x[1], reverse=True)
result = "".join([f"{i+1};{person[0]};{person[1]};{person[2]}\n" for i, person in enumerate(ipeople)])
print(result)